...
class Dashboard extends CI_Controller {
	...
	// metode grafik proses wwt dan pemakaian chemical ================================================================================================
	...
	public function detail($tanggal){
		...
		$this->load->view('template/header');
		$this->load->view('dashboard/detail_netralisasi', $data);
		$this->load->view('template/footer');
	}
	...
	public function detailOutlet($tanggal){
		...
		$this->load->view('template/header');
		$this->load->view('dashboard/detail_outlet', $data);
		$this->load->view('template/footer');
	}

	// metode tampilkan stok asam =====================================================================================================================
	public function stokAsam(){
		...
		//===========================================================================================================================================
		$this->load->view('template/header');
		$this->load->view('dashboard/v_utama2', $data);
		$this->load->view('template/footer');
	}

	// METODE GRAFIK PEMBUATAN ASAM & ASAM RECYCLE ====================================================================================================
	public function PembuatanAsam(){
		...
		//===========================================================================================================================================
		$this->load->view('template/header');
		$this->load->view('dashboard/v_utama3', $data);
		$this->load->view('template/footer');

	}

	// Metode untuk grafik counter air  ===============================================================================================================
	public function counterAir(){
		...
		//===========================================================================================================================================
		$this->load->view('template/header');
		$this->load->view('dashboard/v_utama4', $data);
		$this->load->view('template/footer');
	...
	public function addKaryawan(){
		...
		$data['dataPosisi'] = $this->M_karyawan->getPosisi();

		// var_dump($data['dataPosisi']);die;
		$this->load->view('template/header');
		$this->load->view('dashboard/v_addKaryawan', $data);
		$this->load->view('template/footer');
	}
	...
	public function loadAddKaryawan(){
		...
		$data = array(
			...
			"posisi_id"   		=> $this->input->post("keterangan"),
			"foto_karyawan"   	=> $this->upload->data("file_name")
		);

		// var_dump($data);die; //<- cek data yg diinput berhasil terkirim ga?
	   
		$prosesSimpanKyw = $this->M_karyawan->save_kyw($data);
		if($prosesSimpanKyw == 1){
	...
	public function loadEditKaryawan(){
		...
		//proses upload gambar
			...
			if ( ! $this->upload->do_upload('karyawanFile')){
				...
				$this->M_karyawan->proses_perbaruiKyw($data);
				// $this->load->view('upload_form', $error);
	...
	public function showJadwal(){
		...
		$this->load->view('template/header');
		$this->load->view('dashboard/v_jadwal', $data);
		$this->load->view('template/footer');
	}
	...
	public function addJadwal(){
		...
		$data['dataKaryawan'] 	= $this->M_karyawan->getKaryawan();
		$data['dataShift'] 		= $this->M_shift->getShift();

		// var_dump($data['dataKaryawan']);die;
		$this->load->view('template/header');
		$this->load->view('dashboard/v_addJadwal', $data);
		$this->load->view('template/footer');
	}

	public function loadAddJadwal(){
		// var_dump($_POST);die;
		// Ambil data yang dikirim melalui metode POST
		$jadwalId 		= $this->input->post('id_jadwal');
		$tgl_mulai 		= $this->input->post('date_in');
		$tgl_selesai 	= $this->input->post('date_out');
		$shift_id 		= $this->input->post('shift_id');
		$addmore 		= $this->input->post('addmore');

		// Simpan data karyawan terkait jadwal
		if (!empty($addmore)) {
			foreach ($addmore as $data) {
				$karyawan_id = $data['karyawan_id'];

				$jadwalData = array(
					'id_jadwal' 	=> $jadwalId,
					'date_in' 		=> $tgl_mulai,
					'date_out' 		=> $tgl_selesai,
					'shift_id' 		=> $shift_id,
					'karyawan_id'	=> $karyawan_id
				);

				$this->db->insert('jadwalKerja', $jadwalData);
			}
		}

		// echo 'Data berhasil disimpan';
		redirect('Dashboard/showJadwal');
	}

	//metode untuk menambahkan nilai maks kostik wwt ke db =========================================================================================
	public function set_max_wwt() {
		$value = $this->input->post('nilai_maks_wwt');

		$data = [
			'max_kostik' => $value
		];

		$this->M_utama->setMaxWWT($data);

		redirect('/dashboard');
	}

	//metode untuk menambahkan nilai maks kostik mixbed ke db =======================================================================================
	public function set_maksmixbed(){
		$valueMixbed = $this->input->post('nilai_maksmixbed');

		$data = [
			'max_kostikmixbed' => $valueMixbed
		];

		$this->M_utama->setMaxMixbed($data);

		redirect('/dashboard');
	}

	//metode untuk menambahkan nilai maks kostik demin ke db ========================================================================================
	public function set_maksdemin(){
		$valueDemin = $this->input->post('nilai_maksDemin');

		$data = [
			'max_kostikdemin' => $valueDemin
		];
		
		$this->M_utama->setMaxDemin($data);

		redirect('/dashboard');
	}

	// ===================================================== Menu Sidebar Jadwal OT Kerja ===========================================================
	public function showJadwalOT(){
		$data['dataOT'] 	= $this->M_karyawan->getJadwalOT();
		$data['karyawan'] 	= $this->M_karyawan->getKaryawan();

		// var_dump($data['dataOT']);die;

		$this->load->view('template/header');
		$this->load->view('dashboard/v_jadwalOT', $data);
		$this->load->view('template/footer');
	}

	public function addJadwalOT(){
		$this->load->model("M_karyawan");
		...
		$this->load->view('template/header');
		$this->load->view('dashboard/v_addJadwalOT', $data);
		$this->load->view('template/footer');
	}

	public function loadAddOT(){
		// var_dump($_POST);die;
		// Ambil data yang dikirim melalui metode POST
		$otId 			= $this->input->post('id_ot');
		$tgl_mulai 		= $this->input->post('date_in');
		$tgl_selesai 	= $this->input->post('date_out');
		$jam_mulai 		= $this->input->post('time_in');
		$jam_selesai 	= $this->input->post('time_out');
		...
		if (!empty($addmore)) {
			foreach ($addmore as $data) {
				...
				$this->db->insert('jadwalkerjaot', $jadwalDataOT);
			}
		}

		// echo 'Data berhasil disimpan';
		redirect('Dashboard/showJadwalOT');
	}
