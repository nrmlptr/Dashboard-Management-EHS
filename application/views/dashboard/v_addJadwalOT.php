        ...
        <div class="">
            ...
            <div class="row">
                <div class="col-md-12 col-sm-12">
                    <div class="x_panel">
                        <div class="x_title">
                            <h2>Add Jadwal OT Karyawan - EHS Dept</h2>
                            <ul class="nav navbar-right panel_toolbox">
                                <li><a class="close-link"  href="<?= base_url('Dashboard/showJadwalOT')?>" title="Tombol Untuk Kembali ke Halaman Awal"><i class="fa fa-close"></i></a>
                                </li>
                            </ul>
                            <div class="clearfix"></div>
                        </div>
                        <div class="x_content">
                            <form class="" name="add_name" action="<?= base_url('dashboard/loadAddOT');?>" method="POST" novalidate>
                                <input type="hidden" name="id_ot">
                                ...
                                <div class="field item form-group">
                                    <label class="col-form-label col-md-3 col-sm-3  label-align">Time In<span class="required">*</span></label>
                                    <div class="col-md-2 col-sm-6">
                                        <input class="form-control" type="time" name="time_in" required="required" />
                                    </div>
                                </div>
                                <div class="field item form-group">
                                    <label class="col-form-label col-md-3 col-sm-3  label-align">Time Out<span class="required">*</span></label>
                                    <div class="col-md-2 col-sm-6">
                                        <input class="form-control" type="time" name="time_out" required="required" />
                                    </div>
                                </div>
                                <div class="field item form-group">
                                    <label class="col-form-label col-md-3 col-sm-3  label-align">Total <span class="required">*</span></label>
                                    <div class="col-md-2 col-sm-6">
                                        <input class="form-control" type="number" class='number' name="total" required='required'>
                                    </div>
                                </div>
                                <div class="field item form-group">
                                    <label class="col-form-label col-md-3 col-sm-3  label-align">Alasan OT<span class="required">*</span></label>
...
<script>
    ...
    // Menghitung total secara otomatis saat time_in atau time_out berubah
    $(document).on('change', 'input[name="time_in"], input[name="time_out"]', function() {
        var timeIn = $('input[name="time_in"]').val();
        var timeOut = $('input[name="time_out"]').val();

        if (timeIn && timeOut) {
            var timeInMinutes = getTimeInMinutes(timeIn);
            var timeOutMinutes = getTimeInMinutes(timeOut);

            var totalMinutes = timeOutMinutes - timeInMinutes;

            if (totalMinutes < 0) {
                totalMinutes += 24 * 60; // Add 24 hours in minutes if timeOut is less than timeIn (assumes a 24-hour range)
            }

            var totalHours = Math.floor(totalMinutes / 60);
            var remainingMinutes = totalMinutes % 60;

            $('input[name="total"]').val(totalHours);
            $('input[name="total"]').siblings('.number').val(totalHours); // Update the hidden input field if present
        }
    });
